...
if not os.path.exists("amol_screenshots(5)"):
    os.makedirs("amol_screenshots(5)")
...
try:
    while True:
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    # Save screenshot
                    screenshot_filename = f"Amol_screenshots(4)/Amol_Sharma_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                    cv.imwrite(screenshot_filename, frame)  # Save the screenshot

                    # Log a new entry for amol with screenshot
        ...
        if not recognized:
            ...
            cv.putText(frame, 'Not Amol Sharma', (30, 55), cv.FONT_HERSHEY_SIMPLEX, 1,
                       (255, 0, 0), 2, cv.LINE_AA)
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            df.to_excel('amol_attendance_with_screenshots(5).xlsx', index=False)
            print("DataFrame saved to 'amol_attendance_with_screenshots(5).xlsx'")
    ...
    if not df.empty:
        df.to_excel('amol_attendance_with_screenshots(5).xlsx', index=False)
        print("Final attendance and screenshots saved to 'amol_attendance_with_screenshots(5).xlsx'.")
