...
screenshot_dir = "amol_screenshots(8)"
...
output_dir = 'D:/Infosys Springboard Task/Face_Recognition/atten_score/'
...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                screenshot_filename = f"{screenshot_dir}/{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                if attentive == 'Yes':
                    cv.putText(frame, f'Attentive (Score: {attention_score:.2f})', (10, 30), cv.FONT_HERSHEY_SIMPLEX, 1,
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            df.to_excel(os.path.join(output_dir, 'amol_attendance_with_attention_score(8).xlsx'), index=False)
            print("DataFrame saved to 'amol_attendance_with_attention_score(8).xlsx'")
    ...
    if not df.empty:
        df.to_excel(os.path.join(output_dir, 'amol_attendance_with_attention_score(8).xlsx'), index=False)
        print("Final attendance with attentiveness saved to 'amol_attendance_with_attention_score(8).xlsx'.")
