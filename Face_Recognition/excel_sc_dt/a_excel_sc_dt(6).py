...
if not os.path.exists("amol_screenshots(6)"):
    os.makedirs("amol_screenshots(6)")
...
try:
    while True:
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    ...
                    screenshot_filename = f"Amol_screenshots(6)/Amol_Sharma_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpeg"
                ...
                # Draw a rectangle around the face and label it as Amol Sharma
        ...
        if (datetime.now() - last_saved_time).total_seconds() >= 30:
            df.to_excel('Amol_attendance_with_screenshots_dt(5).xlsx', index=False)
            print("Data saved to 'Amol_attendance_with_screenshots_dt(5).xlsx'.")
    ...
    if not df.empty:
        df.to_excel('Amol_attendance_with_screenshots_dt(5).xlsx', index=False)
        print("Attendance and screenshots saved to 'Amol_attendance_with_screenshots_dt(5).xlsx'.")
